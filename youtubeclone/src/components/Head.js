...
const Head = () => {
  ...
  const getSearchSuggestions = async () => {
    ...
    dispatch(addToCache({ [searchQuery]: json[1] }));
  };

  const handleQuery = (e) => {
    e.preventDefault();
    setSearchTerm(searchQuery); // Update searchTerm with searchQuery
    if (location.pathname === "/search_query") {
      navigate(`/search_query?v=${searchQuery}`, { replace: true });
    } else {
      navigate(`/search_query?v=${searchQuery}`);
    }
  };

  const handleSuggestionClick = (suggestion) => {
    console.log("CLICKED", suggestion);
    navigate(`/search_query?v=${suggestion}`);
    setShowSuggestions(false); // Hide suggestions after clicking
    setSearchQuery(suggestion);
  };

  useEffect(() => {
    // Listen for changes in searchTerm and navigate to the desired page
    if (searchTerm !== "") {
    ...
    <div className="grid grid-flow-col shadow-lg">
      <div className="flex">
        <svg
          ...
          className="h-10 w-12 m-2 py-2 cursor-pointer"
      ...
      <div className="m-2 p-2 relative">
        <form onSubmit={handleQuery}>
          <input
            className="border border-gray-400 w-[32rem] rounded-l-full p-2 place-content-center"
            type="text"
            placeholder="Search"
            value={searchQuery}
          ...
          <button
            type="submit"
            className="border border-gray-400 p-2 rounded-r-full bg-gray-200 w-16"
          >
            🔍
          </button>
        </form>
        ...
        {searchQuery && suggestions.length > 0 && showSuggestions && (
          <div
            ref={suggestionsRef}
            className="absolute mt-2 w-[32rem] bg-white border border-gray-300 rounded-md shadow-md overflow-hidden z-10"
          >
            <ul>
